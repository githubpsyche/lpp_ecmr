...
class CMR(Pytree):
    ...
    def __init__(
        ...
        # Separate LPP transforms capture category-independent and interaction effects.
    ...
    def experience_item(self, item_index: Int_) -> "CMR":
        ...
        base_emotion_strength = jnp.maximum(
            0.0,
            self.emotion_scale * self.is_emotional[self.study_index]
            + self.lpp_main[self.study_index]
            + self.lpp_interaction[self.study_index],
        )
        emcf_lr = lax.cond(
...
def make_factory(
    ...
    class CMRModelFactory:
        def __init__(
            ...
            centered_lpp = lpp_raw - trial_mean
