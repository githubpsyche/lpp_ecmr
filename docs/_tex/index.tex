...
range of stimuli and presentation durations. However, for the LPP to
serve as a useful candidate marker of emotional encoding, it must also
...
Establishing that the LPP is a reliable and behaviorally relevant signal
at the individual and trial level is a necessary precondition for using
it as a model input. The question of whether it actually improves model
predictions was tested separately at the population level. We used these
results to constrain a process model of memory encoding and retrieval.
CMR captures temporal-context encoding and retrieval but has no emotion
mechanism, and so provides a floor. CMR+LPP tests whether the LPP can
serve as a substitute for category labels: it uses the same
single-context architecture as CMR but scales encoding strength by
trial-level LPP amplitude without any emotional context dynamics. eCMR
adds an emotional context layer and emotion-modulated encoding strength,
using only category labels. LPP-eCMR translates the emotion-specific LPP
effect identified by the GLMM into the model's per-item
encoding-strength term. The comparison tests whether trial-level LPP can
substitute for category labels and whether embedding the LPP in a
mechanistic retrieval model improves prediction beyond what category
labels alone provide. We also examine whether the resulting model
generates qualitatively distinct predictions about neural-behavioral
relationships. We assess each model's population-level predictive
validity using pooled fits and examine subject-wise fits secondarily to
characterize individual heterogeneity.
...
encoding strength. CMR+LPP and eCMR each have 10 free parameters,
differing only in whether emotional memory is driven by a continuous
neural signal or categorical source features. eCMR and LPP-eCMR share
identical architecture, differing only in how LPP enters the
encoding-strength term, so differences in predictive performance between
them are attributable to the LPP term alone. Comparing alternatives that
differ in retrieval dynamics would require output-order information that
the present data lack.
...
\subsubsection{eCMR Specification}\label{ecmr-specification}

Building on the behavioural and ERP results reported above, we specify
the eCMR family used to test mechanistic accounts of how the LPP can
modulate encoding within the eCMR framework
(\citeproc{ref-talmi2019retrieved}{Talmi et al., 2019}). Where LPP is
included, it enters as an item-level modulator of emotional encoding
strength.
...
Each item also carries a binary emotion feature vector \(s_{i}\) that
codes emotional category: \(s_{i} = [1, 0]\) for emotional items and
\(s_{i} = [0, 1]\) for neutral items. The emotional context includes
feature dimensions corresponding to the emotional and neutral
categories, so emotional and neutral items cue different
emotional-context states during encoding and retrieval. The eCMR-based
variants share this dual-context architecture; they differ in whether
and how LPP modulates encoding strength within the emotional pathway.
...
\(s_{i}\) & emotion feature vector (2-unit localist) \\
...
Pre-experimental item-to-context associations in \(M^{FC}_{emot}\) are
initialized so that each item cues its fixed emotion feature vector
\(s_i\) with weight \((1 - \gamma)\): emotional items cue the emotional
unit and neutral items cue the neutral unit. \(M^{CF}_{emot}\) is
initialized to zero, so emotional context-to-item associations are
acquired only through encoding.
...
Emotional context drifts toward the feature cued by the presented item
via \(c_{i}^{E,IN} = M^{FC}_{emot}f_{i}\):

\[c_{i}^{E} = \rho_{i}^{E}c_{i - 1}^{E} + \beta_{emot}c_{i}^{E,IN}\]

with \(\rho_{i}^{E}\) computed analogously. Both emotional and neutral
items update emotional context, pushing it toward the feature associated
with that item.
...
drift rate \(\beta_{rec}\). We do not model recall termination
explicitly and instead condition each simulation on the number of items
recalled on that list. We make this scope choice because the present
model comparison is aimed at the structure of item selection within
recalled sets, whereas adding a stopping rule would introduce an
additional submodel that is not central to that comparison. This is
nevertheless a simplification: in free recall, stopping may depend on
retrieval state, and the present analysis does not attempt to model that
dependency.
...
encoding modulation (LPP vs.~category labels). The model architecture
represents a binary emotional-vs-neutral distinction but does not
represent pre-experimental semantic similarity among studied items.
Semantically related and unrelated neutral items were therefore
collapsed (see Materials), which may compress theoretically meaningful
semantic variation. Let \(e_{i}\) be an indicator that equals 1 for
emotional items and 0 for neutral items, and let \(L_{i}\) denote the
trial-mean-centered Early LPP amplitude for item \(i\).

The base model, CMR, has 9 free parameters and includes temporal context
only. No emotional context layer is included and no emotion parameters
are estimated.

CMR+LPP, with 10 free parameters, uses the same single-context
architecture as CMR but adds one parameter \(\kappa_{L}\) scaling
trial-level Early LPP amplitude into encoding strength:
...
eCMR, also with 10 free parameters, takes a different approach: rather
than using continuous neural measures, it parameterizes
emotion-modulated encoding strength with a category-based term
\(\phi_{emot}\) within the emotional context architecture described
above:
...
LPP-eCMR, with 11 free parameters, is a hypothesis-driven extension in
which one parameter \(\kappa_{EL}\) scales the Early LPP's contribution
to emotional encoding strength:

\[\phi_{emot,i} = \phi_{emot} \cdot e_{i} + \kappa_{EL} \cdot L_{i} \cdot e_{i}\]

This specification operationalizes the earlier GLMM result that Early
LPP amplitude predicted recall for emotional but not neutral items
(\(\beta\) = .14, \emph{p} \textless{} .001 for emotional; \(\beta\) =
.01, \emph{p} = .488 for neutral). The \(e_{i}\) term therefore
restricts the LPP's influence to emotional items, and no main-effect LPP
term is included. A matched variant in which LPP is allowed to affect
all items was not fit here. The emotional-only restriction is therefore
a hypothesis-driven modelling choice motivated by the earlier analysis,
not a conclusion established by the model comparison itself.
...
(LPP vs.~category labels). With 10 free parameters each, the comparison
between CMR+LPP and eCMR tests whether trial-level LPP can replace
category labels at equal parameter cost.
...
items recalled from each list (not recall order), because our data is
...
on list \(t\), and let \(r\) range over recall sequences that contain
...
list we draw \(K\) random permutations \(r_{1},\ldots,r_{K}\) of
...
the optimizer is stochastic, we run three independent fits per model
variant and retain the best-performing fit. We compare models using AIC
computed from the approximate set log-likelihood.

The primary model comparisons use pooled fits, in which a single shared
parameter vector is fit to all 342 lists across 38 subjects. Pooled fits
target population-level predictive validity. They also avoid the
small-sample problem that arises in subject-wise fits: with only 9 lists
per subject and 9--11 free parameters, the standard AICc correction
(\citeproc{ref-burnham2002model}{Burnham \& Anderson, 2002}) is
unavailable because \(n \le k+1\). Subject-wise (no-pooling) fits are
reported secondarily to characterize heterogeneity. Unlike pooled fits,
which ask whether a single shared parameter vector predicts recall sets
well at the population level, subject-wise fits ask how model
performance varies when each participant is allowed their own parameter
vector (\citeproc{ref-gelman2006data}{Gelman \& Hill, 2006}). For nested
model pairs in the subject-wise analysis, occasional optimizer failures
can make the more flexible model fit worse than its simpler parent for a
given subject, even though the models are nested. When that occurred, we
replaced the more flexible model's negative log-likelihood with the
parent model's value before computing pairwise comparisons. Pairwise
\(\Delta\)AIC with 95\% CIs are reported for the key subject-wise
contrasts.

We assess qualitative adequacy by simulating each fitted model on the
study lists while matching the number of items recalled on each list and
plotting benchmark summaries. This set-likelihood approach evaluates
models on their ability to reproduce the full recalled sets, providing a
stringent test of mechanistic hypotheses.
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image3.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image4.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image5.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image6.png}}
...
For the LPP to be useful as a model constraint, it needs to relate to
memory performance at a relevant level of analysis. We therefore
examined that relation at the between-subject, within-subject, and
single-trial levels in turn. We first investigated whether the
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image7.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image8.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image9.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/media/image10.png}}
...
\paragraph{Pooled Analysis.}\label{pooled-analysis}

Table~\ref{tbl-pooled} summarizes the primary pooled model comparison,
in which a single parameter vector per model was fit to all 342 lists.
eCMR dominated both single-context models (\(\Delta\)AIC = 67.26 over
CMR, 51.46 over CMR+LPP), supporting the eCMR specification tested here
over the single-context alternatives. CMR+LPP improved on CMR
(\(\Delta\)AIC = 15.80), indicating that the LPP carries a
population-level signal about encoding strength even without an
emotional context layer. However, a single-context model driven by LPP
did not substitute for the eCMR specification tested here: with 10 free
parameters each, eCMR substantially outperformed CMR+LPP.

LPP-eCMR achieved the best fit overall, improving on eCMR by
\(\Delta\)AIC = 34.16. Supplementing category labels with trial-level
LPP improved pooled predictive fit.

\begin{table}

{\caption{{Pooled model comparison. \(k\) = free parameters; \(-\)LL =
negative log-likelihood for a single shared parameter vector fit to all
342 lists across 38 subjects; \(\Delta\)AIC relative to best
model.}{\label{tbl-pooled}}}
\vspace{-20pt}}

\begin{longtable}[]{@{}lcccc@{}}
\toprule\noalign{}
Model & \(k\) & \(-\)LL & AIC & \(\Delta\)AIC \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\paragraph{Subject-Wise Analysis.}\label{subject-wise-analysis}

Separate subject-wise (no-pooling) fits were also examined. Because each
subject contributed only 9 lists for models with 9--11 free parameters,
these comparisons are methodologically fragile and are not treated as
the primary inferential basis; they are reported to characterize
heterogeneity. Nesting flooring was required for 23 of 152
subject--model entries.

\begin{table}

{\caption{{Subject-wise model comparison. \(k\) = free parameters; NLL =
negative log-likelihood (mean \(\pm\) SE across 38
participants).}{\label{tbl-comparison}}}
\vspace{-20pt}}

\begin{longtable}[]{@{}lcc@{}}
\toprule\noalign{}
Model & \(k\) & Mean NLL (\(\pm\) SE) \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
CMR & 9 & 215.01 \(\pm\) 15.63 \\
CMR+LPP & 10 & 213.84 \(\pm\) 15.73 \\
eCMR & 10 & 211.98 \(\pm\) 16.31 \\
LPP-eCMR & 11 & 210.83 \(\pm\) 16.46 \\
\end{longtable}

\end{table}

The broad ordering of models was similar, but the LPP-related gains were
weak and heterogeneous at the subject level. eCMR outperformed CMR (mean
\(\Delta\)AIC = 4.06 {[}1.36, 6.77{]}). However, mean \(\Delta\)AIC was
0.33 {[}\(-0.77\), 1.44{]} for CMR+LPP over CMR and 0.30 {[}\(-0.70\),
1.29{]} for LPP-eCMR over eCMR, with both CIs including zero.

\paragraph{Qualitative Diagnostics.}\label{qualitative-diagnostics}

We assessed qualitative adequacy with two benchmark diagnostics
(Figure~\ref{fig-empirical}; Figure~\ref{fig-catspc};
Figure~\ref{fig-lpp-recall}). The first targets the emotional
enhancement of memory established in the Behavioural Results.
Category-specific serial position curves (Figure~\ref{fig-catspc}) show
that CMR, which lacks an emotional context layer, predicts nearly
identical recall for negative and neutral items. CMR+LPP produces a
faint separation because LPP amplitude correlates weakly with emotional
category, but the effect is far weaker than what the eCMR-based
specifications achieve. Both eCMR and LPP-eCMR reproduce the empirical
separation: their shared emotional context ensures that negative items
promote one another's recall, lifting the negative SPC above the neutral
baseline.

A second diagnostic targets the LPP--recall link established in the EEG
Results. Empirically, recalled emotional items had higher Early LPP
amplitudes than unrecalled emotional items; no such difference existed
for neutral items. eCMR cannot reproduce this pattern: it assigns
identical encoding strength to all emotional items regardless of their
LPP amplitude, so simulated LPP distributions for recalled and
unrecalled emotional items overlap. Both CMR+LPP and LPP-eCMR reproduce
the separation, since both use LPP to modulate encoding strength. Higher
LPP leads to stronger encoding and higher recall probability
(Figure~\ref{fig-lpp-recall}). The critical difference is that only
LPP-eCMR also captures the category-level recall structure visible in
the SPC diagnostic. This recalled-versus-unrecalled Early LPP separation
was not directly fit: the models were optimized to predict recalled
sets, not this neural-behavioral contrast. In that sense, LPP-eCMR
generalizes beyond the fitting target, although the model form was
informed by prior empirical results. LPP-eCMR captures a mechanistic
link (LPP \(\rightarrow\) encoding strength \(\rightarrow\) recall
probability) that produces the observed neural-behavioral association as
a consequence of its architecture, while also reproducing the
categorical emotional memory advantage.

The LPP thus improves pooled predictive fit and generates mechanistic
predictions that were not used as fitting targets. However, it does not
substitute for the eCMR specification tested here: CMR+LPP remains far
behind eCMR, and subject-wise gains are small and heterogeneous under
current data constraints. The value of incorporating trial-level neural
measures lies in the population-level predictive improvement and in
generating qualitatively distinct predictions about neural-behavioral
relationships. One possible interpretation is that trial-to-trial Early
LPP variation is more behaviorally meaningful for emotional items
because it more strongly indexes affective significance or motivated
attention during encoding. A weaker relationship for neutral items could
still exist, but under the present data it may be too small or too noisy
to detect.

\begin{figure}

\caption{\label{fig-empirical}Empirical benchmark diagnostics. Left:
category-specific serial position curves showing the
negative-above-neutral recall advantage. Right: Early LPP amplitude by
recall status for negative items showing the recalled/unrecalled
separation.}

\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/analyses/reference_catspc.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/analyses/cat_lpp_by_recall_EarlyLPP_Negative.png}}\end{minipage}%

\end{figure}%

\begin{figure}

\caption{\label{fig-catspc}Simulated category-specific serial position
curves. Top: CMR (left), CMR+LPP (right). Bottom: eCMR (left), LPP-eCMR
(right). CMR predicts nearly identical curves; CMR+LPP produces a faint
separation via indirect LPP-category correlation; eCMR and LPP-eCMR
reproduce the full negative-above-neutral separation (compare
Figure~\ref{fig-empirical}).}

\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_CMR_50_set_likelihood_fixed_term_best_of_3_cat_spc_sim.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_LPP_CMR_50_set_likelihood_fixed_term_best_of_3_cat_spc_sim.png}}\end{minipage}%
\newline
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_eCMR_50_set_likelihood_best_of_3_cat_spc_sim.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_LPP_eCMR_50_set_likelihood_best_of_3_cat_spc_sim.png}}\end{minipage}%

\end{figure}%

\begin{figure}

\caption{\label{fig-lpp-recall}Simulated Early LPP amplitude by recall
status for negative items. Top: CMR (left), CMR+LPP (right). Bottom:
eCMR (left), LPP-eCMR (right). CMR+LPP and LPP-eCMR reproduce the
recalled/unrecalled separation; CMR and eCMR cannot (compare
Figure~\ref{fig-empirical}).}

\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_CMR_50_set_likelihood_fixed_term_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_LPP_CMR_50_set_likelihood_fixed_term_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png}}\end{minipage}%
\newline
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_eCMR_50_set_likelihood_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_LPP_eCMR_50_set_likelihood_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png}}\end{minipage}%
