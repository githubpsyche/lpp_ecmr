...
drift rate \(\beta_{rec}\). We omit an explicit termination mechanism
and instead match each trial's observed output length during simulation.
Conditioning on observed output length isolates the model's predictions
about which items are recalled from predictions about how many, which is
appropriate because our primary behavioral outcomes and benchmarks
target item selection rather than stopping behavior.
...
represents a binary emotional-vs-neutral distinction and does not
capture within-neutral semantic substructure. Semantically related and
unrelated neutral items were therefore collapsed (see Materials), which
may compress theoretically meaningful within-neutral variation. Let
\(e_{i}\) be an indicator that equals 1 for emotional items and 0 for
neutral items, and let \(L_{i}\) denote the trial-mean-centered Early
LPP amplitude for item \(i\).
...
parameter vector is fit to all 342 trials across 38 subjects. Pooled
fits target population-level predictive validity. They also avoid the
small-sample problem that arises in subject-wise fits: with only 9
trials per subject and 9--11 free parameters, the standard AICc
correction (\citeproc{ref-burnham2002model}{Burnham \& Anderson, 2002})
is unavailable because \(n \le k+1\). Subject-wise (no-pooling) fits are
reported secondarily to characterize heterogeneity. They answer a
different question from pooled fits rather than the same question at
different noise levels (\citeproc{ref-gelman2006data}{Gelman \& Hill,
2006}). For nested model pairs in the subject-wise analysis, we floor
the child model's per-subject NLL at the parent model's value to correct
for occasional optimizer failures that would otherwise violate the
nesting guarantee. Pairwise \(\Delta\)AIC with 95\% CIs are reported for
the key subject-wise contrasts.

We assess qualitative adequacy by simulating each fitted model on the
study lists while matching each trial's observed output length and
...
in which a single parameter vector per model was fit to all 342 trials.
eCMR dominated both single-context models (\(\Delta\)AIC = 67.26 over
CMR, 51.46 over CMR+LPP), confirming that the dual-context mechanism
captures meaningful structure in emotional recall beyond temporal
dynamics alone. CMR+LPP improved on CMR (\(\Delta\)AIC = 15.80),
indicating that the LPP carries a population-level signal about encoding
strength even without an emotional context layer. However, the LPP alone
cannot substitute for category labels: at equal parameter cost
(\(k = 10\)), eCMR substantially outperformed CMR+LPP.
...
342 trials across 38 subjects; \(\Delta\)AIC relative to best
...
category, but the effect is far weaker than what the emotional context
architecture achieves. Both eCMR and LPP-eCMR reproduce the empirical
...
Results. The two models also make different predictions about an
untrained neural-behavioral summary statistic. Empirically, recalled
emotional items had higher Early LPP amplitudes than unrecalled
emotional items; no such difference existed for neutral items,
consistent with the GLMM. eCMR cannot reproduce this pattern: it assigns
...
the SPC diagnostic. This outcome reflects genuine generalization since
the models are fit to set likelihood, not to the
...
substitute for the emotional context architecture: CMR+LPP remains far
...
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_WeirdCMRNoStop_group_level_best_of_3_cat_spc_sim.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_EEGLPPParsimonious_group_level_best_of_3_cat_spc_sim.png}}\end{minipage}%
\newline
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_eCMREmotionOnly_group_level_best_of_3_cat_spc_sim.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_eCMREmotionTimesLPP_group_level_best_of_3_cat_spc_sim.png}}\end{minipage}%
...
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_WeirdCMRNoStop_group_level_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_EEGLPPParsimonious_group_level_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png}}\end{minipage}%
\newline
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_eCMREmotionOnly_group_level_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png}}\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fitting/TalmiEEG_eCMREmotionTimesLPP_group_level_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png}}\end{minipage}%
