...
We omit an explicit termination mechanism and instead match each trial's observed output length during simulation.
Conditioning on observed output length isolates the model's predictions about which items are recalled from predictions about how many, which is appropriate because our primary behavioral outcomes and benchmarks target item selection rather than stopping behavior.
...
The model architecture represents a binary emotional-vs-neutral distinction and does not capture within-neutral semantic substructure.
Semantically related and unrelated neutral items were therefore collapsed (see Materials), which may compress theoretically meaningful within-neutral variation.
...
The primary model comparisons use pooled fits, in which a single shared parameter vector is fit to all 342 trials across 38 subjects.
Pooled fits target population-level predictive validity.
They also avoid the small-sample problem that arises in subject-wise fits: with only 9 trials per subject and 9--11 free parameters, the standard AICc correction [@burnham2002model] is unavailable because $n \le k+1$.
Subject-wise (no-pooling) fits are reported secondarily to characterize heterogeneity.
They answer a different question from pooled fits rather than the same question at different noise levels [@gelman2006data].
For nested model pairs in the subject-wise analysis, we floor the child model's per-subject NLL at the parent model's value to correct for occasional optimizer failures that would otherwise violate the nesting guarantee.
Pairwise $\Delta$AIC with 95% CIs are reported for the key subject-wise contrasts.

We assess qualitative adequacy by simulating each fitted model on the study lists while matching each trial's observed output length and plotting benchmark summaries.
...
@tbl-pooled summarizes the primary pooled model comparison, in which a single parameter vector per model was fit to all 342 trials.
eCMR dominated both single-context models ($\Delta$AIC = 67.26 over CMR, 51.46 over CMR+LPP), confirming that the dual-context mechanism captures meaningful structure in emotional recall beyond temporal dynamics alone.
...
However, the LPP alone cannot substitute for category labels: at equal parameter cost ($k = 10$), eCMR substantially outperformed CMR+LPP.
...
Pooled model comparison. $k$ = free parameters; $-$LL = negative log-likelihood for a single shared parameter vector fit to all 342 trials across 38 subjects; $\Delta$AIC relative to best model.
::::
...
The two models also make different predictions about an untrained neural-behavioral summary statistic.
...
However, it does not substitute for the emotional context architecture: CMR+LPP remains far behind eCMR, and subject-wise gains are small and heterogeneous under current data constraints.
...
![](figures/fitting/TalmiEEG_WeirdCMRNoStop_group_level_best_of_3_cat_spc_sim.png)

![](figures/fitting/TalmiEEG_EEGLPPParsimonious_group_level_best_of_3_cat_spc_sim.png)

![](figures/fitting/TalmiEEG_eCMREmotionOnly_group_level_best_of_3_cat_spc_sim.png)

![](figures/fitting/TalmiEEG_eCMREmotionTimesLPP_group_level_best_of_3_cat_spc_sim.png)
...
![](figures/fitting/TalmiEEG_WeirdCMRNoStop_group_level_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png)

![](figures/fitting/TalmiEEG_EEGLPPParsimonious_group_level_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png)

![](figures/fitting/TalmiEEG_eCMREmotionOnly_group_level_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png)

![](figures/fitting/TalmiEEG_eCMREmotionTimesLPP_group_level_best_of_3_cat_lpp_by_recall_NEGATIVE_EARLYLPP_sim.png)
